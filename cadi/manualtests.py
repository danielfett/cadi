...
class RPManualTest:
    ...
    important_security_check: bool = False
    important_acceptance_check: bool = False
...
RP_MANUAL_TESTS = {
    ...
    "Security / ID Token": {
        "m110_id_token_nonce": RPManualTest(
            ...
            "Only applicable in the identity flow when the nonce parameter is used.",
        ...
        "m120_id_token_expired": RPManualTest(
            ...
            "Only applicable in the identity flow.",
            acceptance_condition="The ID Token must not be accepted.",
            how_to_fix="Check that you have implemented all ID Token checks as per the developer guide.",
    ...
    "Security / Authorization Response": {
        ...
        "m202_iss_twice": RPManualTest(
            name="Two Issuer Identifiers",
            description="The `iss` parameter in the authorization response is sent twice, the correct value and a wrong value. This simulates a mistake by an IDP.",
            acceptance_condition="The authorization response should not be accepted.",
            how_to_fix="Check that you only accept one `iss` parameter in the authorization response.",
        ),
    ...
    "Security / Authentication": {
        "m300_acr_wrong": RPManualTest(
            ...
            "This test is only applicable if you ask for 2FA in your request via `acr_values`.",
        ...
        "m310_acr_missing": RPManualTest(
            ...
            "This test is only applicable if you ask 2FA in your request via `acr_values`.",
            acceptance_condition="If 2FA is critical for the use case, the ID Token must not be accepted.",
            how_to_fix="Always check that the `acr` parameter exists and is set to the value required for your use case. ",
            requires=lambda session: ACRValues.SCA in session.acr_values_list,
            important_security_check=True,
    ...
    "User Experience": {
        "m800_user_aborts": RPManualTest(
            ...
            important_acceptance_check=True,
        ),
